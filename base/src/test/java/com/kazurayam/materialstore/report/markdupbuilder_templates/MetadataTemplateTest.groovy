...
class MetadataTemplateTest extends AbstractTemplateTest {
    ...
    void test_getCSSClassName_identified_value() {
        QueryOnMetadata query = QueryOnMetadata.builder(AbstractTemplateTest.metadata0).build()
        IdentifyMetadataValues identifyMetadataValues =
                new IdentifyMetadataValues.Builder()
                        .putAllNameRegexPairs(["profile": "MyAdmin_ProductionEnv"])
                        .build()
        String cssClassName =
                new MetadataTemplate(AbstractTemplateTest.metadata0)
    ...
    void test_getCSSClassNameSolo() {
        QueryOnMetadata query = QueryOnMetadata.builder(AbstractTemplateTest.metadata0).build()
        String cssClassName =
                new MetadataTemplate(AbstractTemplateTest.metadata0).getCSSClassNameSolo(query, "profile")
        assertEquals("matched-value", cssClassName)
    }
    ...
    void test_toSpanSequence_Metadata_dual_QueryOnMetadata() {
        ...
        new MetadataTemplate(AbstractTemplateTest.metadata0).toSpanSequence(mb, query)
...
<span>}</span>
    ...
    void test_toSpanSequence_Metadata_single_QueryOnMetadata() {
        ...
        StringWriter sw = new StringWriter()
        MarkupBuilder mb = new MarkupBuilder(sw)
        new MetadataTemplate(AbstractTemplateTest.metadata0).toSpanSequence(
