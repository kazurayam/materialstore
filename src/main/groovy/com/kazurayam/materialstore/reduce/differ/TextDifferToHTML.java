...
import com.kazurayam.materialstore.report.StyleHelper;
...
public final class TextDifferToHTML extends AbstractTextDiffer implements Differ, HTMLPrettyPrintingCapable {
    ...
    public TextDiffContent makeTextDiffContent(Store store,
        ...
        String leftText = AbstractTextDiffer.readMaterial(store, left, charset);
        String rightText = AbstractTextDiffer.readMaterial(store, right, charset);

        //build simple lists of the lines of the two text files
        List<String> leftLines = AbstractTextDiffer.readAllLines(leftText);
        List<String> rightLines = AbstractTextDiffer.readAllLines(rightText);
        ...
        model.put("style", StyleHelper.loadStyleFromClasspath("/com/kazurayam/materialstore/reduce/differ/style.css"));
    ...
    /*
    public static void markupSegments(final MarkupBuilder mb, final List<String> segments) {
        nospace(mb);
        LinkedHashMap<String, String> map = new LinkedHashMap<String, String>(1);
        map.put("class", "blob-code-inner");
        ((MarkupBuilder) mb).span(map, new Closure<List<String>>(null, null) {
            public List<String> doCall(Object it) {
                final Reference<Boolean> inOldTag = new Reference<boolean>(false);
                final Reference<Boolean> inNewTag = new Reference<boolean>(false);
                return DefaultGroovyMethods.each(segments, new Closure<Boolean>(null, null) {
                    public Boolean doCall(String segment) {
                        if (StringGroovyMethods.isCase(OLD_TAG, segment)) {
                            setGroovyRef(inOldTag, !inOldTag.get());
                        } else if (StringGroovyMethods.isCase(NEW_TAG, segment)) {
                            setGroovyRef(inNewTag, !inNewTag.get());
                        } else {
                            if (inOldTag.get()) {
                                nospace(mb);
                                LinkedHashMap<String, String> map1 = new LinkedHashMap<String, String>(1);
                                map1.put("class", "deletion");
                                ((MarkupBuilder) mb).span(map1, segment);
                            } else if (inNewTag.get()) {
                                nospace(mb);
                                LinkedHashMap<String, String> map1 = new LinkedHashMap<String, String>(1);
                                map1.put("class", "insertion");
                                ((MarkupBuilder) mb).span(map1, segment);
                            } else {
                                nospace(mb);
                                LinkedHashMap<String, String> map1 = new LinkedHashMap<String, String>(1);
                                map1.put("class", "unchanged");
                                ((MarkupBuilder) mb).span(map1, segment);
                            }
    ...
    */
    ...
    /*
    public static void nospace(MarkupBuilder mb) {
        mb.getMetaClass().setAttribute(mb, "nospace", true);
    }
    */

    public static String getCLASS_TD_CHANGE() {
        return CLASS_TD_CHANGE;
    }

    public static String getCLASS_TD_DELETE() {
        return CLASS_TD_DELETE;
    }

    public static String getCLASS_TD_EQUAL() {
        return CLASS_TD_EQUAL;
    }

    public static String getCLASS_TD_INSERT() {
